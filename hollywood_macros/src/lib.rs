...
// The macros are documented in the root-level hollywood crate - so that links are generated
// correctly.
#[allow(missing_docs)]
...
#[allow(missing_docs)]
#[proc_macro_attribute]
pub fn actor_in_requests(attr: TokenStream, item: TokenStream) -> TokenStream {
    core::actor_in_requests_impl(
        proc_macro2::TokenStream::from(attr),
        proc_macro2::TokenStream::from(item),
    )
    .into()
